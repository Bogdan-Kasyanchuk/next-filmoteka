...
import { getCurrentMovieById, getRecommendationsMovies } from '@/services/tmdbApi/movies';
    ...
    return generateMetaTags(
        {
            ...
            description: `Recommended movies based on the ${ title }.`,
            keywords: [ title, `recommended to ${ title }` ],
            url: recommendationsUrl(MediaType.MOVIE, params.id)
        }
    );
}
...
export default async function Page(props: Props) {
    ...
    await Promise.all([
        ...
        await queryClient.prefetchQuery(
            {
                queryKey: [ 'movies', params.id, 'recommendations', page ],
                queryFn: () => getRecommendationsMovies(params.id, page)
            }
        )
    ]);
